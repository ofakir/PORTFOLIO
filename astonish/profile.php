...
<?php 
    ...
    if(!isset($_SESSION['user_id']))
        ...
        $profile_image_query = mysqli_query($conn, "SELECT profile_image FROM user_form WHERE user_id = '$user_id'");
        $profile_image_row = mysqli_fetch_assoc($profile_image_query);
        $profile_image = null;
        if (!empty($profile_image_row['profile_image'])) {
            $finfo = finfo_open(FILEINFO_MIME_TYPE);
            $mime_type = finfo_buffer($finfo, $profile_image_row['profile_image']);
            finfo_close($finfo);
            if (!$mime_type) {
                $mime_type = 'image/png'; // Default MIME type
            }
            $profile_image = 'data:' . $mime_type . ';base64,' . base64_encode($profile_image_row['profile_image']);
        } else {
        ...
        <div class="middle-container container" style="margin-left: 380px; margin-top: -380px;">
            <div class="profile block"> <!-- PROFILE (MIDDLE-CONTAINER) -->
                ...
                <div class="profile-picture big-profile-picture clear">
                    <img width="150px" alt="user-profile" src="<?php echo htmlspecialchars($profile_image, ENT_QUOTES, 'UTF-8'); ?>">
                </div>
                <h1 class="user-name"><?php echo $_SESSION['user_name']; ?></h1>
                <div class="profile-description">
                    <p class="scnd-font-color">Welcome To Astonish ! </p>
                </div>
                <a class="subscribe button" href="edit-profile.php" style="color: white; margin-top: 10px;">Edit Profile</a>                
